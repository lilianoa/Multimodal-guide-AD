    ...
    def train_epoch(self, model, dataset, optimizer, epoch, mode='train'):
        ...
        for i, batch in enumerate(batch_loader):
            ...
            if i % Options()['engine']['print_freq'] == 0:
                ...
                '''
                Logger()("{} img_loss: {:.5f}".format(' ' * len(mode), out['img_loss'].data.item()))
                Logger()("{} text_loss: {:.5f}".format(' ' * len(mode), out['text_loss'].data.item()))
                Logger()("{} fu_loss: {:.5f}".format(' ' * len(mode), out['fu_loss'].data.item()))
                '''
...
class KDEngine(Engine):
    ...
    def t_resume(self, map_location=None):
        """ Resume a teacher network using the `bootstrap.lib.options.Options`
        """
        Logger()('Loading {} teacher network from {}'.format(Options()['imp']['resume'], Options()['imp']['dir']))
        self.t_load(Options()['imp']['dir'],
                  Options()['imp']['resume'],
                  self.model.teachernet,
                  map_location=map_location)

    def t_load(self, dir_logs, name, model, map_location=None):
        """ Load a teacher network
        ...
        model_state = torch.load(path_template.format(name, 'model'), map_location=map_location)
        for k in list(model_state['network'].keys()):
            if k.startswith('classif_v') or k.startswith('classif_t'):
                del model_state['network'][k]
