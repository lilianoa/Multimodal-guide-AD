...
def factory_img_enc(opt):
    if opt['name'] == 'clip':
        if os.path.isfile(opt['type']):
            ...
            model = getattr(models, opt['type'])(pretrained=True)
            img_enc = model
    else:
        img_enc = None
...
class TeacherNet(nn.Module):
    ...
    def forward(self, batch):
        ...
        logits = self.classif(fusion_feature)
        out = {'logits': logits, 'feats': fusion_feature}
        return out

    def init_parameters(self):
        '''
        for name, param in self.visual.named_parameters():
            if 'layer4' in name or 'attnpool' in name:
