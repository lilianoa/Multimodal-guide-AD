...
class BanOptimizer():
    ...
    def step(self):
        ...
        # Logger().log_value('train_batch.norm', self.total_norm.item() / self.count_norm)
...
class BanOptimizer_sp(BanOptimizer):
    def __init__(self, engine,
                 name='Adamax',
                 lr=0.0007,
                 gradual_warmup_steps=[0.5, 2.0, 4],
        ...
        self.engine = engine
        self.optimizer = torch.optim.__dict__[name](
            list(filter(lambda p: p.requires_grad, engine.model.img_net.parameters())) +
            list(filter(lambda p: p.requires_grad, engine.model.text_net.parameters())) +
            list(filter(lambda p: p.requires_grad, engine.model.fu_net.parameters())),
            lr=lr
        )
        self.lr_decay_rate = lr_decay_rate
        self.lr_decay_epochs = eval("range({},{},{})".format(*lr_decay_epochs))
...
class BanOptimizer_model(BanOptimizer):
